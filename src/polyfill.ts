...
function contains(s: any, ss: string): boolean { return String(s).indexOf(ss) !== -1; }
...
function mergeIf(baseTable: any, select: string, table: any) {
...
var keyCodeToInfoTable: {[index: number]: {code: string, keyCap?: string, location?: number}} = {
    // 0x01 - VK_LBUTTON
    // 0x02 - VK_RBUTTON
...
var keyIdentifierTable: {[index: string]: { code: string, location?: number }} = {};
if ('cros' === os) {
...
var locationTable: {[index: number]: { code: string, location?: number, keyCap?: string }}[] = [];
locationTable[LEFT] = {
...
var codeToKeyTable: {[index: string]: {key: string, shiftKey?: string}} = {
    // Modifier Keys
    ShiftLeft: { key: 'Shift' },
...
export function keyInfoForEvent(event: KeyboardEvent<HTMLElement>) {
...
export function queryKeyCap(code: any, locale: any) {
    code = String(code);
